...
private:
    ...
    bool loadNvsWifiParam(void);
    bool setNvsWifiParam(char *ssid, size_t ssid_size, char *password, size_t password_size);
    ...
    std::map<std::string, std::string> _nvs_wifi_param_map;
    // std::map<std::string, std::variant<int32_t, std::string, float>> _nvs_param_map;
