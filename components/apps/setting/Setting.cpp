...
#define NVS_WIFI_STORAGE_NAMESPACE      "wifi_storage"
...
bool AppSettings::init(void)
{
    ...
    _nvs_wifi_param_map[NVS_KEY_WIFI_SSID] = std::string("SEEED-MKT");
    _nvs_wifi_param_map[NVS_KEY_WIFI_PASSWORD] = std::string("edgemaker2024");
...
bool AppSettings::loadNvsWifiParam(void)
{
    esp_err_t err = ESP_OK;
    nvs_handle_t nvs_handle;
    err = nvs_open(NVS_WIFI_STORAGE_NAMESPACE, NVS_READWRITE, &nvs_handle);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Error (%s) opening NVS handle!", esp_err_to_name(err));
        return false;
    }

    for (auto& key_value : _nvs_wifi_param_map) 
    {
        size_t required_size = 0;

        err = nvs_get_str(nvs_handle, key_value.first.c_str(), nullptr, &required_size);
        if (err == ESP_OK && required_size > 0) {
            char* buffer = new char[required_size]; 
            err = nvs_get_str(nvs_handle, key_value.first.c_str(), buffer, &required_size);
            if (err == ESP_OK) 
            {
                key_value.second = std::string(buffer); //  std::string
        ...
        else if (err == ESP_ERR_NVS_NOT_FOUND) 
        {
            ...
            ESP_LOGW(TAG, "The value of %s is not initialized yet, set it to default value: %s",
...
bool AppSettings::setNvsWifiParam(char *ssid, size_t ssid_size, char *password, size_t password_size)
{
    nvs_handle_t nvs_handle;
    esp_err_t err = nvs_open(NVS_WIFI_STORAGE_NAMESPACE, NVS_READWRITE, &nvs_handle);
    if (err != ESP_OK) {
        ESP_LOGE("NVS", "Failed to open NVS handle: %s", esp_err_to_name(err));
        return false;
    }

    err = nvs_set_str(nvs_handle, NVS_KEY_WIFI_SSID, reinterpret_cast<const char*>(ssid));
    if (err != ESP_OK) {
        ESP_LOGE("NVS", "Failed to store SSID: %s", esp_err_to_name(err));
        return false;
    }

    err = nvs_set_str(nvs_handle, NVS_KEY_WIFI_PASSWORD, reinterpret_cast<const char*>(password));
    if (err != ESP_OK) {
        ESP_LOGE("NVS", "Failed to store password: %s", esp_err_to_name(err));
        return false;
    }
...
void AppSettings::wifiConnectTask(void *arg)
{
    ...
    if (bits & WIFI_EVENT_CONNECTED) {
        ...
        app->setNvsWifiParam(st_wifi_ssid, strlen(st_wifi_ssid),st_wifi_password, strlen(st_wifi_password));
